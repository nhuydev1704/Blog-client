...
import { CALLBACK } from '../../redux/types/callbackType';

const CardBlog = (props: any) => {
    const blogs = get(props, 'blogs', []);
    const userBlog = get(props, 'userBlog', false);

    const { auth, callback } = useSelector((state: RootStore) => state);
    ...
    const handleDeleteBlog = async (blog: any) => {
        ...
        await dispatch({ type: CALLBACK, payload: !callback });
    };
    ...
    return (
        <Row gutter={[16, 16]} style={{ marginTop: '10px' }}>
            {blogs &&
                ...
                blogs.map((item: any) => (
                    <Col
                        xl={props.layout ? 12 : 6}
                        md={props.layout ? 24 : 12}
                        xs={24}
                        lg={props.layout ? 24 : 12}
                        key={item._id}
                    >
                        <Card
                            style={{
                            ...
                            <Row gutter={[0, 6]}>
                                <Col span={24}>
                                    ...
                                    {typeof item.user !== 'string' && (
                                        <Row gutter={[0, 16]} style={{ marginTop: '4px' }}>
                                            <Col span={14}>
                                                {item.user && (
                                ...
                                <Col span={24}>
                                    {auth &&
                                        auth.user &&
                                        userBlog &&
                                        (item.user as IUser)?._id === (auth.user as IUser)?._id && (
