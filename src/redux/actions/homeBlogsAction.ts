...
export const createBlog =
    (dataBlog: IBlog, token: string) => async (dispatch: Dispatch<IAlertType | ICreateBlogsByUserType>) => {
...
export const deleteBlog =
    (blog: IBlog, token: string) => async (dispatch: Dispatch<IAlertType | IdeleteBlogsByUserType>) => {
        const result = await checkTokenExp(token, dispatch);
        const access_token = result ? result : token;
        try {
            dispatch({ type: ALERT, payload: { loading: true } });
...
export const getBlogsByCategory =
    (id: string, search: string) => async (dispatch: Dispatch<IAlertType | IGetBlogsByCategoryType>) => {
        try {
            let limit = 4;
...
export const getBlogsByUser =
    (id: string, search: string) => async (dispatch: Dispatch<IAlertType | IGetBlogsByUserType>) => {
        try {
            let limit = 4;
            let pageValue = search ? search : `?page=${1}`;
            dispatch({ type: ALERT, payload: { loading: true } });
